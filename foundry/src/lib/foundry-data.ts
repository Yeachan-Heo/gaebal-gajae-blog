...
export const tokenGroups: readonly TokenGroup[] = [
  {
    ...
    tokens: [
      ...
      { token: 'surface-raised', preview: 'surface', usage: { ko: '강조 카드 / docs preview', en: 'Lifted cards and docs previews', zh: '抬起卡片 / docs 预览', ja: '持ち上げたカード / docs プレビュー' }, description: { ko: '본문보다 한 단계 올라오되 액센트보다 먼저 소리치지 않는 강조 표면.', en: 'A lifted surface that rises above prose without speaking before the accent.', zh: '比正文抬高一层、但不会先于 accent 发声的强调表面。', ja: '本文より一段上がりながら、accent より先に叫ばない強調面。' } },
      { token: 'surface-selected', preview: 'surface', usage: { ko: '현재 선택 / 활성 상태', en: 'Selected and active state', zh: '当前选择 / 激活状态', ja: '現在選択 / アクティブ状態' }, description: { ko: '현재 위치와 선택 상태를 분명하게 보여 주되 페이지 전체를 칠하지 않는 상태 표면.', en: 'Clear selected-state surface that marks current location without painting the whole page.', zh: '清楚标记当前位置、但不会涂满整页的选中状态表面。', ja: '現在位置を明確にしながら、ページ全体を塗らない選択状態の面。' } },
      { token: 'surface-nav', preview: 'surface', usage: { ko: '상단 인덱스 바', en: 'Editorial index bar', zh: '编辑索引条', ja: '編集インデックスバー' }, description: { ko: '짧은 링크와 조작을 안정적으로 담는 상단 인덱스 바.', en: 'Stable top index bar for short links and controls.', zh: '稳定承载短链接与控制项的顶部编辑索引条。', ja: '短いリンクと操作を安定して収める上部インデックスバー。' } },
