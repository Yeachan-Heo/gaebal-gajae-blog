...
export function renderHomeBody({
  ...
  return [
    ...
    `<section class="section home-recent-section"><div class="section-head"><div><h2>${localizedBlock(latestTitle)}</h2><p class="section-description">${localizedBlock(latestBody)}</p></div>${renderActionLink({ href: '/archive', label: localizedText(ui.allPosts), i18nKey: 'allPosts' })}</div><div class="home-recent-list">${recentRows}</div></section>`,
...
export function renderLaneIndexBody({
  ...
  const leadBlock = lead
    ? `<section class="section lane-featured lane-featured-${laneKey}"><div class="section-head"><div><p class="kicker" data-i18n="featured">${localizedText(ui.featured, 'ko')}</p><h2>${localizedBlock(lane.title)}</h2><p class="section-description">${localizedBlock(laneIntro.focus)}</p></div>${linkedLocalizedText('/archive', laneIntro.cta, localizedBlock)}</div>${featuredPostCard(lead)}</section>`
    : '';
